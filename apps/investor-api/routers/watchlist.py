...
def get_watchlist(payload: dict = Depends(investor_user), db: Session = Depends(get_db)):
    user_id = payload["sub"]

    profile = db.query(InvestorProfile).filter(InvestorProfile.userId == user_id).first()
    if not profile:
        return {"profile": None, "watchlist": []}

    matches = (
        db.query(DealMatch)
        .join(Deal,           DealMatch.dealId   == Deal.id)
        .join(StartupProfile, Deal.startupId     == StartupProfile.id)
        .filter(DealMatch.investorId == profile.id, DealMatch.status == "interested")
