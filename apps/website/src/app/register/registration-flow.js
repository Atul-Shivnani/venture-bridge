...
function Field({ label, required, hint, children }) {
  return (
    <div>
      ...
      {children}
      {hint && <p className="mt-1.5 text-xs text-ink-soft/60">{hint}</p>}
    </div>
  );
}
...
function StartupForm({ formState, onChange }) {
  return (
    <div className="space-y-5">
      ...
      <Field label="Short Description" required hint="One or two sentences about what your company does.">
        <textarea
      ...
      <Field label="Business Model" required hint="e.g. SaaS, marketplace, transactional">
        <input
          type="text"
      ...
      <Field label="Customers Served" hint="Describe your target customer or current customer base.">
        <input
          type="text"
          value={formState.customersServed}
          onChange={(e) => onChange("customersServed", e.target.value)}
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Amount Raising (USD)" required hint="Total raise target in USD">
          <input
            type="number"
        ...
        <Field label="Instrument Type" required hint="e.g. SAFE, Equity, Convertible Note">
          <input
            type="text"
      ...
      <Field label="Use of Funds" required hint="How will you deploy the capital raised?">
        <textarea
...
function InvestorForm({ formState, onChange }) {
  return (
    <div className="space-y-5">
      ...
      {/* Geography + Sectors */}
      <Field label="Geography Allowed" required hint="Comma-separated regions or countries.">
        <input
          type="text"
          value={formState.geography}
          onChange={(e) => onChange("geography", e.target.value)}
          placeholder="e.g. India, Southeast Asia, MENA"
          required
          className={inputCls}
        />
      </Field>

      <Field label="Sector Allowed" hint="Comma-separated sectors you invest in.">
        <input
          type="text"
      ...
      {/* Stage + Ticket */}
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Stage Allowed" hint="Comma-separated stages.">
          <input
            type="text"
            value={formState.stagesAllowed}
            onChange={(e) => onChange("stagesAllowed", e.target.value)}
            placeholder="e.g. pre-seed, seed, series-a"
            className={inputCls}
          />
        </Field>
        <Field label="Ticket Size Range" required hint='e.g. "$250k – $2M"'>
          <input
            type="text"
      ...
      {/* Instrument type */}
      <Field label="Instrument Type Allowed" hint="Comma-separated instrument types.">
        <input
          type="text"
          value={formState.instrumentTypesAllowed}
          onChange={(e) => onChange("instrumentTypesAllowed", e.target.value)}
          placeholder="e.g. SAFE, Equity, Convertible Note"
          className={inputCls}
        />
      </Field>

      {/* Revenue floor + profitability */}
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Revenue Floor (USD)" hint="Minimum annual revenue required, if any.">
          <input
            type="number"
            value={formState.revenueFloor}
            onChange={(e) => onChange("revenueFloor", e.target.value)}
            placeholder="e.g. 100000"
            min="0"
            className={inputCls}
          />
        </Field>
        <Field label="Profitability Preference">
          <SelectWrapper>
            <select
              value={formState.profitabilityPreference}
              onChange={(e) => onChange("profitabilityPreference", e.target.value)}
              className={selectCls}
            >
              <option value="">No preference</option>
              <option value="pre-revenue-ok">Pre-revenue OK</option>
              <option value="path-to-profitability">Path to profitability</option>
              <option value="profitable">Must be profitable</option>
            </select>
          </SelectWrapper>
        </Field>
      </div>

      {/* ESG + Regulatory exclusions */}
      <Field label="ESG Exclusions" hint="Comma-separated sectors or activities to exclude for ESG reasons.">
        <input
          type="text"
          value={formState.esgExclusions}
          onChange={(e) => onChange("esgExclusions", e.target.value)}
          placeholder="e.g. Tobacco, Weapons, Gambling"
          className={inputCls}
        />
      </Field>

      <Field label="Regulatory Exclusions" hint="Comma-separated regulatory restrictions or jurisdictions to exclude.">
        <input
          type="text"
          value={formState.regulatoryExclusions}
          onChange={(e) => onChange("regulatoryExclusions", e.target.value)}
          placeholder="e.g. Crypto, Cannabis, OFAC countries"
          className={inputCls}
        />
      </Field>
    </div>
      ...
      const details =
        portal === "investor"
          ? {
              ...
              stagesAllowed: formState.stagesAllowed,
              instrumentTypesAllowed: formState.instrumentTypesAllowed,
              revenueFloor: formState.revenueFloor,
              profitabilityPreference: formState.profitabilityPreference,
              esgExclusions: formState.esgExclusions,
              regulatoryExclusions: formState.regulatoryExclusions,
            }
          : {
              companyName: formState.companyName,
              sector: formState.sector,
              subSector: formState.subSector,
              ...
              customersServed: formState.customersServed,
              amountRaising: formState.amountRaising,
